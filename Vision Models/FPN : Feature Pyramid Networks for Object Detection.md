...
##### 예시
입력 해상도가 256×256×3인 이미지가 ResNet 같은 Backbone 네트워크를 통과할 때, FPN의 두 경로는 다음과 같이 작동합니다. 

##### Bottom-up Pathway (상향식 경로)
일반적인 CNN의 순방향 전파 과정입니다. 층을 거칠 때마다 해상도는 1/2로 줄어들고, 채널(의미 정보)은 더 깊고 복잡해집니다. 
- C2 레이어: 64×64 (해상도 1/4배) – 비교적 정교한 위치 정보 보유
- C3 레이어: 32×32 (해상도 1/8배)
- C4 레이어: 16×16 (해상도 1/16배)
- C5 레이어: 8×8 (해상도 1/32배) – 가장 추상적이고 강력한 의미 정보 보유

##### Top-down Pathway & Lateral Connection (하향식 및 측면 연결)
가장 높은 층(C5)의 풍부한 정보를 아래로 전달하며, 각 층의 해상도를 2배씩 키워(Upsampling) 하위 층의 정보와 결합합니다. 
- 시작: 8×8 크기의 C5에 1×1 합성곱을 적용해 채널을 256으로 맞춘 P5를 만듭니다.

###### 1단계: 입력 텐서의 구조 정의 
먼저 Backbone(예: ResNet-50)의 가장 마지막 블록에서 추출된 특징 맵 $\(C5\)$ 의 크기를 정의합니다.  
입력 이미지가 $\(256\times 256\times 3\)$ 일 때, 일반적으로 32배 다운샘플링된 $\(C5\)$ 는 다음과 같은 크기를 가집니다. 
- 높이 $(\(H\))$ : $\(8\)$
- 너비 $(\(W\))$ : $\(8\)$
- 채널 $(\(C_{in}\))$ : $\(2048\)$ (ResNet-50 기준)

따라서 입력 텐서의 형태는 $\((8,8,2048)\)$ 입니다.

- 전달: P5를 2배 업샘플링하여 16×16 크기로 만든 뒤, 16×16 크기인 C4와 더합니다(Lateral Connection).
이때 C4도 1×1 합성곱을 통해 채널을 256으로 맞춰줍니다.

###### 2단계: $\(1\times 1\)$ 합성곱(Convolution)의 역할 이해 
$\(1\times 1\)$ 합성곱은 공간적 해상도 $(\(H,W\))$ 는 유지하면서 채널 수 $(\(C\))$ 만 조절하는 데 최적화된 연산입니다.  
FPN에서는 모든 피라미드 층 $(\(P2\)~\(P5\))$ 의 채널을 동일하게 맞춰야 이후 단계에서 측면 연결(Lateral Connection) 시 덧셈 연산이 가능해집니다. 
- 필터(Kernel) 크기: $\(1\times 1\)$
- 출력 채널 수 $(\(C_{out}\))$ : $\(256\)$ (FPN 논문 표준 설정값)

###### 3단계: 연산 과정 및 출력 크기 계산 
각 위치(pixel)에서 $\(2048\)$ 개의 채널 데이터가 $\(256\)$ 개의 서로 다른 $\(1\times 1\)$ 필터와 내적(Dot Product)을 수행합니다. 
- 공간 해상도 계산: 스트라이드(Stride)가 1이므로 출력 크기는 유지됩니다. : $\(H_{out}=\frac{8-1+2(0)}{1}+1=8\)$
- 파라미터 수: 각 필터는 $\(1\times 1\times 2048\)$ 의 가중치를 가지며, 총 256개의 필터가 사용됩니다.
- 최종 텐서 형태: 출력 채널이 $\(256\)$ 이 되도록 고정했으므로 최종 결과는 다음과 같습니다. : $\(\text{Output\ Shape}=(8\times 8\times 256)\)$

> 직접 결합 (Element-wise Addition) 🌟

> 두 특징 맵의 크기 $(\(16\times 16\))$ 와 채널 $(\(256\))$ 이 완벽히 일치하게 설정되어야 가능합니다. 이때 두 값을 그대로 더합니다.

> 식: $\(P4_{temp}=(C4\text{\ after\ }1\times 1\text{\ Conv})+(\text{Upsampled\ }P5)\)$

> 결과: $\(16\times 16\times 256\)$ (각 픽셀들끼리 값만 더함)

> 정보의 융합: 하위 층의 정교한 위치 정보와 상위 층의 풍부한 의미 정보를 같은 공간에 중첩(Superimpose)시키는 효과가 있습니다.

##
- 반복: 위 과정을 거쳐 32×32(P3), 64×64(P2) 특징 맵을 순차적으로 생성합니다.
- 정제: 마지막으로 각 결과물에 3×3 합성곱을 적용하여 업샘플링으로 인한 왜곡(Aliasing)을 제거하고 최종 특징 피라미드를 완성합니다.

이 구조 덕분에 해상도가 높으면서도 상위 층의 의미 정보를 모두 포함하고 있어, 작은 물체를 아주 정확하게 찾아낼 수 있게 됩니다.
