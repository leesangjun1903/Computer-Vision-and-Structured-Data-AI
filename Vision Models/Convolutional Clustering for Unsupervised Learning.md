...
#### 일반적인 k-평균 적용 (패치 수준 학습)
- 목표: 무작위로 추출된 이미지 패치들로부터 딕셔너리(필터)를 학습하는 것입니다 (p. 3).
- 과정:
  - 이미지에서 작은 패치들을 무작위로 추출합니다 (p. 3).
  - 이 패치들을 데이터 포인트로 사용하여 k-평균 클러스터링 알고리즘을 실행합니다.
  - 알고리즘의 결과로 얻어지는 클러스터 중심이 곧 네트워크의 필터가 됩니다 (p. 3).
- 한계: 이 방식은 학습된 필터들이 공간적으로 이동된 중복된 버전을 많이 포함하게 되어 효율성이 떨어집니다 (p. 3).
...
#### 제안하는 컨볼루션 k-평균 적용 (윈도우 수준 학습)
- 목표: 필터 간의 중복성을 줄이고 다양한 특징을 학습하는 것입니다 (p. 4).
- 과정:
  - 먼저 필터 크기보다 두 배 큰 윈도우를 무작위로 선택합니다 (p. 4).
  - 학습 중인 필터(클러스터 중심)를 전체 윈도우에 컨볼루션 연산하여 유사성 점수를 계산합니다 (p. 4). 유사성은 기본적으로 내적 (dot product)을 계산하여 측정합니다. 이는 필터(클러스터 중심) 벡터와 입력 이미지 패치 벡터 사이의 유사성 또는 선형 상관관계를 나타냅니다. 알고리즘은 윈도우 내에서 필터 $\(D^{(l)}\)$ 와 가장 큰 활성화(가장 큰 내적 값)를 보이는 패치 위치를 찾습니다 (p. 4). 즉, 이 방식은 특정 필터가 윈도우 영역 어디에 존재하든 가장 잘 일치하는 위치를 찾아 유사성을 극대화합니다.
  - 가장 높은 활성화 점수를 얻은 위치의 패치만을 추출하여 해당 필터의 새로운 데이터 포인트로 사용합니다 (p. 4).
- 결과: 이 방법을 통해 학습된 필터는 기존 k-평균 필터보다 훨씬 더 다양하며 중복성이 현저히 줄어듭니다 (p. 4).
...
모델은 다중 계층 신경망 구조를 사용하며, 각 계층은 이전 계층의 출력(특징 맵)을 입력으로 받아들이면서 점점 더 복잡하고 추상적인 특징을 학습합니다.
##### 첫 번째 계층 (Pixel Level Features):
첫 번째 계층의 필터는 원본 이미지의 픽셀 데이터에 직접 컨볼루션 방식으로 적용됩니다.  
이 단계에서 학습되는 필터는 주로 가장자리, 선, 코너와 같은 기본적이고 저수준의 시각적 특징을 감지합니다. 예를 들어, 그림 1a와 1b는 수평 가장자리 필터가 학습된 것을 보여줍니다.
##### 두 번째 및 그 이상의 계층 (Mid-level to High-level Features):
두 번째 계층부터는 입력 이미지가 아닌, 첫 번째 계층에서 생성된 특징 맵(활성화된 저수준 특징들의 집합)을 입력으로 받습니다.  
이 계층의 필터는 저수준 특징들의 조합에서 패턴을 찾기 시작합니다. 예를 들어, 여러 개의 가장자리 특징이 모여 텍스처나 단순한 형태(shapes)를 형성하는 것을 학습할 수 있습니다.  
네트워크의 깊이가 깊어질수록(3계층), 필터는 이러한 중간 수준의 특징들을 다시 조합하여 더 크고 복잡한 객체의 부분(parts of objects)이나 전체 객체를 인식하는 고수준 특징으로 발전합니다.
